...
/*function reports_branch_load(pageno){
    ...
    remote_type=json.response[0].obranches[0].remotetype;
	var branch_str="";
	PAGENO=pageno;
	if(pageno===1){
...
function dashboard_reportsdate(){
	/*var index=document.getElementById("branches").value;
	test_localstorage=localStorage.getItem("resultobject");
    json = JSON.parse(test_localstorage);*/
	var Startdate=document.getElementById("sdatepicker").value;
	var Enddate=document.getElementById("edatepicker").value;
	if( Startdate===""||Enddate===""){
		noty( {"text":"date should not be empty...","layout":"bottomRight","type":"error"});
		return false;
	}
	var objectForPost = null;
	var contentType = 'application/json; charset=utf-8';
	var servletPath = 'reportsdate';
	var method = "POST";
		objectForPost = {	
...
function dashboardCallback(resultobject){
	/******Home Page DashBoard start******/
	if(resultobject.Daywiserequestdata.length!==0){
	document.getElementById("daywisedata").innerHTML="";
	document.getElementById("mostusedtable").innerHTML="";
	document.getElementById("autocancel").innerHTML="";
	document.getElementById("monthwise").innerHTML="";
	...
	for(var i=0;i<resultobject.tablewiserequestData.length;i++){
		...
		tablewiserequest_service[i]=resultobject.tablewiserequestData[i].nurse;
		tablewiserequest_drink[i]=resultobject.tablewiserequestData[i].sos;
	...
	for(var i=0;i<tablewiserequest_tableno.length;i++){
        ...
        alltabledata_donutwhite(tablewiserequest_tableno[i],tablewiserequest_service[i],tablewiserequest_drink[i],tablewiserequest_bill[i]);
		
	}
	
	}
	else{
		noty( {"text":"No data between this date...","layout":"bottomRight","type":"error"});
	}
}
...
function alltabledata_donut_white(DivID,serviceCount,drinkCount,billCount,likecount){
	...
	$('#'+DivID).highcharts({
		 chart: {
	        ...
	        tooltip: {
	            pointFormat: 'TableNo: <b>{point.y}</b>'
	        },
	        exporting: { enabled: false },
	        plotOptions: {
	            pie: {
...
function alltabledata_donutwhite(DivID,serviceCount,drinkCount,billCount){
	...
	$('#'+DivID).highcharts({
        ...
        series: [{
            name: 'BedNo - '+DivID,
            data: [
                ['Sos : '+drinkCount, drinkCount],
                ['Nurse : '+serviceCount, serviceCount],
